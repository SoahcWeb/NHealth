<x-app-layout>
    <x-slot name="header">
        <x-nhealth.page-header eyebrow="Objectifs NHealth" title="Modifier l’objectif">
            <x-slot name="action">
    ...
    <div class="nhealth-shell-form">
        <x-nhealth.flash :message="session('status')" title="Objectif mis à jour" />

        @if ($errors->any())
            <x-nhealth.alert type="error" title="Problème de validation">
                Les modifications de l’objectif ne sont pas encore complètes. Vérifiez les champs en surbrillance avant d’enregistrer.
            </x-nhealth.alert>
        @endif

        <div class="grid gap-6 lg:grid-cols-[1fr,auto]">
            <x-nhealth.section eyebrow="Construction de l’objectif" title="Mettre à jour l’objectif">
                        ...
                        <div>
                            ...
                            <select id="status" name="status" class="nhealth-select">
                                @foreach (['draft' => 'Brouillon', 'active' => 'Actif', 'paused' => 'En pause', 'completed' => 'Terminé', 'archived' => 'Archivé'] as $status => $label)
                                    <option value="{{ $status }}" @selected(old('status', $goal->status) === $status)>{{ $label }}</option>
                                @endforeach
                            </select>
            ...
            <div class="panel h-fit lg:w-64">
                ...
                <p class="mt-2 text-sm leading-6 text-slate-400">
                    Supprimez cet objectif uniquement s’il n’est plus utile à votre historique de suivi privé.
                </p>
                <form method="POST" action="{{ route('nhealth.goals.destroy', $goal) }}">
                    @csrf
                    @method('DELETE')
                    <div class="mt-4">
                        <x-danger-button>Supprimer l’objectif</x-danger-button>
                    </div>
                </form>
            </div>
        </div>
